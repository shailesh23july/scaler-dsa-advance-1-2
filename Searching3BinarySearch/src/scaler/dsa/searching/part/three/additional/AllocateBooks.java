package scaler.dsa.searching.part.three.additional;
/*
Problem Description
...
public class AllocateBooks {
	...
	public static void main(String[] args) {
		int[] A = {12, 34, 67, 90};
		int B = 4;
		int allocateBooks = allocateBooks(A, B);
		System.out.println(allocateBooks);
	...
	private static boolean isAllocationPossible(int[] A, int m, int B) {
		int studentAllocated = 1;
		int pagesAllocated = 0;
		for (int i = 0; i < A.length; i++) {
			if (pagesAllocated + A[i] <= m) {
				pagesAllocated += A[i];
			} else {
				pagesAllocated = A[i];
				studentAllocated++;
			}
			if (studentAllocated > B) {
				return false;
			}
		}
		return true;
